...
class TestCodeAudit:
    ...
    # Excluded from Phase 1 audit: Phase 3+ intentionally introduces real connectors here.
    AUDIT_EXCLUDE = [
        ROOT / "cloud" / "data",
    ]
    ...
    def _collect_py_files(self) -> list[Path]:
        ...
        for p in self.AUDIT_PATHS:
            for f in p.rglob("*.py"):
                if not any(f.is_relative_to(excl) for excl in self.AUDIT_EXCLUDE):
                    files.append(f)
        return files

    def test_no_deep_learning_framework_imports(self):
        ...
        assert not violations, "Deep-learning framework code found:\n" + "\n".join(violations)
